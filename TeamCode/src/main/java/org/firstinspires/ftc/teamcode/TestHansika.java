...
public class TestHansika extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        GamepadController gamepadController = new GamepadController(gamepad1);
...
//            telemetry.addData("isDown", gamepadReader.getButton("one").isDown() ? "1" : "0");
            ...
            if (gamepadController.getButton("one").wasJustReleased()) {
                telemetry.addData("hasReleased", "1");
                telemetry.update();
            }
            if (gamepadController.getButton("one").wasJustPressed()) {
