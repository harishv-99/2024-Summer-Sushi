package edu.ftcsushi.fw.assisteddriving.shape;

import com.acmerobotics.roadrunner.*;
import com.acmerobotics.roadrunner.Math;

import edu.ftcsushi.fw.util.MathEx;
...
public class Segment implements Shape {
    ...
    public Segment(Vector2d start, Vector2d end) {
        ...
        lineNormalized = MathEx.normalize(lineTemp);
    }
    ...
    public RelativeLocation getRelativeLocationOfPoint(Vector2d point) {
        ...
        //       just pick a side.
        if (MathEx.det(end.minus(start), point.minus(start)) > 1)
    ...
    public Vector2dDual<Arclength> getClosestPoint(Vector2d point) {
        ...
        return MathEx.createPointTangent(closestPoint, lineNormalized);
    }
    ...
    public Vector2dDual<Arclength> getPointIntersectedBy(Pose2d pose) {
        ...
        Vector2d S = MathEx.normalize(pose.heading.vec());
        ...
        return MathEx.createPointTangent(X, lineNormalized);
    }
    ...
    public Rotation2d getClosestHeading(Pose2d pose) {
        ...
        double angleToStart = MathEx.angleToTurn(pose.heading.vec(), poseToStart);
        double angleToEnd = MathEx.angleToTurn(pose.heading.vec(), poseToEnd);
